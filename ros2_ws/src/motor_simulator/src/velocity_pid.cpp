...
public:
  ...
  lastdata(0.0)
  {
    torque_publisher_ = this->create_publisher<std_msgs::msg::Float64>("motor_simulator_control_input", 10);
    ve_subscription_ = this->create_subscription<std_msgs::msg::Float64>("simulated_motor_velocity", 10,
                            std::bind(&velocity_pid::ve_subscriber_callback, this, std::placeholders::_1));
    angle_subscription_ = this->create_subscription<std_msgs::msg::Float64>("simulated_motor_angle", 10,
...
private:
//滤波器代码
  double lowfilter_process(double data)
  {
    double nowdata = data;
    double res;
    double a = 0.8;   //低通滤波系数
  ...
  void ve_subscriber_callback(const std_msgs::msg::Float64::SharedPtr msg)
  {
    ...
    auto delta_time = std::chrono::duration_cast<std::chrono::milliseconds>(
            ...
            velocity = lowfilter_process(msg->data);
  ...
  pid velo_pid{10,0.0f,0.0f,0.020,0.002,0.00};
  pid angle_pid{50,0.0f,M_PI/2,4.0,0.00,0.00};
